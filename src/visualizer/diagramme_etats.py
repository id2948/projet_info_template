...
def rounded_box(ax, cx, cy, w, h, title, lines, color,
                title_fs=10, body_fs=8.5, lw=1.3):
    """Boîte avec titre gras et liste de lignes de contenu."""
    rect = FancyBboxPatch(
        (cx - w/2, cy - h/2), w, h,
        boxstyle="round,pad=0.06,rounding_size=0.10",
        facecolor=color, edgecolor="#37474F", linewidth=lw, zorder=3,
    )
    ax.add_patch(rect)
    ...
    ax.text(cx, ty, title, ha="center", va="center",
            fontsize=title_fs, color=FW, fontweight="bold", zorder=4)
    if lines:
        sep_y = cy + h/2 - row_h
        ax.plot([cx - w/2 + 0.05, cx + w/2 - 0.05], [sep_y, sep_y],
        ...
        for i, line in enumerate(lines):
            ...
            ax.text(cx - w/2 + 0.18, ly, "• " + line,
                    ha="left", va="center", fontsize=body_fs,
                    color=FW, zorder=4, alpha=0.93)
...
def dot_end(ax, cx, cy, r=0.14):
    ax.add_patch(plt.Circle((cx, cy), r, color=C_INIT, zorder=5))
    ax.add_patch(plt.Circle((cx, cy), r + 0.09, color="none",
                             ec=C_INIT, lw=2.0, zorder=5))

def arrow(ax, x1, y1, x2, y2, label="", color=ARROW, rad=0.0,
          lw=1.5, ls="-", label_dx=0.08, label_dy=0.0, label_ha="left"):
    ...
    if label:
        mx = (x1+x2)/2 + label_dx
        my = (y1+y2)/2 + label_dy
        ax.text(mx, my, label, fontsize=7.5, color=color, ha=label_ha, va="center",
...
def band(ax, y0, h, label, color="#F5F5F5", ec="#CFD8DC"):
    ax.add_patch(FancyBboxPatch(
        ...
        facecolor=color, edgecolor=ec, lw=0.7, zorder=0, alpha=0.55
    ))
    ax.text(0.28, y0 + h - 0.07, label, fontsize=7, color="#78909C",
...
# ── Titre ─────────────────────────────────────────────────────────────────────
ax.text(7, 21.65, "Diagramme d'états et de fonctionnement",
        ha="center", va="center", fontsize=16, fontweight="bold", color=C_INIT)
ax.text(7, 21.28, "Application Sportive  ·  5 sports : Basketball · Football · LoL · Tennis · Volleyball",
...
# Menu Sport
rounded_box(ax, 2.5, 19.98, 4.0, 0.52,
            "MENU SPORT",
            ["basketball · football · LOL · tennis · volley"],
            C_MENU, title_fs=10, body_fs=8)

# → q / FIN
arrow(ax, 4.5, 19.98, 5.5, 19.98, label="q", color="#546E7A",
      label_dx=0.06, label_ha="left")
...
# → valide descend
arrow(ax, 2.5, 19.72, 2.5, 19.42, label="sport valide",
      label_dx=0.1, label_ha="left")

# Menu Catégorie
rounded_box(ax, 2.5, 19.14, 4.6, 0.52,
            "MENU CATÉGORIE",
            ["match · joueur · equipe · competition · graphiques · ajouter · historique"],
            C_MENU, title_fs=10, body_fs=7.8)

# → q / FIN
arrow(ax, 4.8, 19.14, 5.5, 19.14, label="q", color="#546E7A",
      label_dx=0.06, label_ha="left")
dot_end(ax, 5.72, 19.14)
ax.text(6.0, 19.14, "FIN", fontsize=8.5, color=C_INIT, va="center", fontweight="bold")

# retour → menu sport
arrow(ax, 0.5, 19.14, 0.5, 19.72, color="#546E7A", rad=0.0, lw=1.1,
      label="retour", label_dx=0.08)
...
arrow(ax, 2.5, 18.88, 7.0, 18.82, color=ARROW, lw=1.2, ls="dashed",
      label="catégorie choisie", label_dx=0.1)
...
for cx, title, lines, col in R1_BOXES:
    ...
    by = R1Y - bh/2 + 0.38
    rounded_box(ax, cx, by, 2.65, bh, title, lines, col,
                title_fs=9.5, body_fs=8)
    # sortie affichage
    note_y = by - bh/2 - 0.20
    ax.text(cx, note_y, "→ affichage terminal",
            ha="center", va="center", fontsize=7.5, color=C_OUT,
            bbox=dict(boxstyle="round,pad=0.18", fc="#E8F5E9", ec="#66BB6A", lw=0.8))
    # retour
    arrow(ax, cx, note_y - 0.18, cx, note_y - 0.45, color="#546E7A", lw=1.0)
    ax.text(cx, note_y - 0.55, "↺ retour menu catégorie",
            ha="center", va="center", fontsize=6.8, color="#546E7A")
...
for cx, title, lines, col in R2_BOXES:
    ...
    by = R2Y - bh/2 + 0.42
    rounded_box(ax, cx, by, 2.80, bh, title, lines, col,
                title_fs=9.5, body_fs=8.0)
    # sorties spécifiques
    note_y = by - bh/2 - 0.20
    if "GRAPHIQUES" in title:
        ax.text(cx, note_y, "→ sauvegarde  output/*.png",
                ha="center", va="center", fontsize=7.5, color=C_GRAPH,
        ...
        ax.text(cx, note_y,
                ...
                ha="center", va="center", fontsize=7.2, color=C_ERR,
    ...
    ax.text(cx, note_y - 0.60, "↺ retour menu catégorie",
            ha="center", va="center", fontsize=6.8, color="#546E7A")
...
for xd, folder, content in data:
    ...
    rect = FancyBboxPatch(
        (xd - 1.0, 6.85), 2.0, 2.65,
        boxstyle="round,pad=0.07,rounding_size=0.08",
        facecolor=fc, edgecolor=ec, linewidth=1.2, zorder=3,
    )
    ax.add_patch(rect)
    ax.text(xd, 9.17, folder, ha="center", va="center",
            fontsize=8.5, fontweight="bold", color="#BF360C" if is_res else C_DATA, zorder=4)
    ax.plot([xd-0.90, xd+0.90], [8.93, 8.93], color=ec, lw=0.6, zorder=4)
    ax.text(xd, 8.20, content, ha="center", va="center",
            fontsize=7.2, color=FD, zorder=4, linespacing=1.4)
...
rounded_box(ax, 3.5, 5.10, 5.5, 1.60,
            "TERMINAL (stdout)",
            [
            ...
            C_DATA, title_fs=10, body_fs=8.2)

rounded_box(ax, 10.5, 5.10, 5.5, 1.60,
            "FICHIERS  output/*.png",
            [
...
ax.text(7.0, 3.65,
        ...
        ha="center", va="center", fontsize=8.0, color=C_DATA,
...
handles = [mpatches.Patch(facecolor=c, edgecolor="#37474F", label=l) for c, l in légende]
ax.legend(handles=handles, loc="lower center", ncol=5,
          fontsize=8, frameon=True, edgecolor="#CFD8DC", facecolor=BG,
