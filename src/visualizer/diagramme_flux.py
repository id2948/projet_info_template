...
def box(ax, cx, cy, w, h, texte, fs=8.5, bold=False):
    ...
    ax.text(cx, cy, texte, ha="center", va="center",
            fontsize=fs, color=TXT, fontweight=fw, zorder=4,
            linespacing=1.35)
...
def oval(ax, cx, cy, w, h, texte, fs=9, bold=True):
    ax.add_patch(Ellipse((cx, cy), w, h,
                          facecolor=FC, edgecolor=EC, linewidth=LW, zorder=3))
    ax.text(cx, cy, texte, ha="center", va="center",
            fontsize=fs, color=TXT, fontweight="bold" if bold else "normal", zorder=4)

def arr(ax, x1, y1, x2, y2, label="", rad=0.0,
        label_side="mid", lfs=7.2, lw=LW):
    """Flèche avec label positionné au milieu ou décalé."""
    ax.annotate("", xy=(x2, y2), xytext=(x1, y1),
                arrowprops=dict(
                    arrowstyle="-|>", color=ARR, lw=lw,
    ...
    if label:
        if label_side == "mid":
            ...
            mx = (x1 + x2) / 2; my = (y1 + y2) / 2; dx = -0.12; dy = 0
        else:
            mx = (x1 + x2) / 2; my = (y1 + y2) / 2; dx = 0.12; dy = 0
        ax.text(mx + dx, my + dy, label, ha="left" if dx >= 0 else "right",
                va="center", fontsize=lfs, color=LBL, zorder=5,
                bbox=dict(boxstyle="round,pad=0.10", fc=BG, ec="none", alpha=0.9))

def label_on_arrow(ax, x, y, texte, fs=7.2, ha="left"):
    ax.text(x, y, texte, ha=ha, va="center", fontsize=fs, color=LBL, zorder=5,
            bbox=dict(boxstyle="round,pad=0.10", fc=BG, ec="none", alpha=0.9))
...
# ── Titre ─────────────────────────────────────────────────────────────────────
ax.text(7, 19.70, "Diagramme de fonctionnement — Application Sportive",
...
ax.annotate("", xy=(6.0, 18.94), xytext=(6.0, 18.65),
            arrowprops=dict(arrowstyle="-|>", color=ARR, lw=LW,
                            connectionstyle="arc3,rad=0.0"), zorder=2)
ax.annotate("", xy=(6.0, 18.65), xytext=(8.0, 18.65),
            arrowprops=dict(arrowstyle="-", color=ARR, lw=LW), zorder=2)
ax.annotate("", xy=(8.0, 18.65), xytext=(8.0, 18.84),
            arrowprops=dict(arrowstyle="-", color=ARR, lw=LW), zorder=2)
...
label_on_arrow(ax, 7.08, 17.05, "L'utilisateur choisit une catégorie ou « retour »", fs=7.0)

# ══════════════════════════════════════════════════════════════════════════════
# NIVEAU 2 — MENU CATÉGORIE
# ══════════════════════════════════════════════════════════════════════════════

box(ax, 7.0, 16.46, 6.0, 0.50,
    "Menu catégorie\n(match · joueur · equipe · competition · graphiques · ajouter · historique)",
    fs=8, bold=True)

# retour → menu principal
ax.annotate("", xy=(1.4, 18.85), xytext=(1.4, 16.46),
...
CATS    = ["match", "joueur", "equipe", "compé-\ntition", "gra-\nphiques", "ajouter", "histo-\nrique"]
...
ax.annotate("", xy=(4.0, 16.21), xytext=(1.0, y_last_m - 0.78),
            arrowprops=dict(arrowstyle="-|>", color=ARR, lw=0.9,
...
box(ax, 8.4, 13.65, 1.80, 0.46,
    "L'utilisateur saisit\nle n° du graphique", fs=7.3)

arr(ax, 8.4, 13.42, 8.4, 12.96)
box(ax, 8.4, 12.73, 1.80, 0.46,
    "Paramètres demandés\nsi besoin (ligue, saison…)", fs=7.3)

arr(ax, 8.4, 12.50, 8.4, 12.04)
box(ax, 8.4, 11.82, 1.80, 0.44,
    "Génération\ndu graphique PNG", fs=7.3)

arr(ax, 8.4, 11.60, 8.4, 11.14)
box(ax, 8.4, 10.92, 1.80, 0.44,
    "Sauvegarde\noutput/*.png", fs=7.3)

arr(ax, 8.4, 10.70, 8.4, 10.24)
box(ax, 8.4, 10.02, 1.80, 0.44,
    "Affichage\ndu chemin fichier", fs=7.3)

# → retour
ax.annotate("", xy=(7.0, 16.21), xytext=(8.4, 9.80),
            arrowprops=dict(arrowstyle="-|>", color=ARR, lw=0.9,
...
for i, step in enumerate(ajouter_steps):
    ...
    box(ax, 10.5, yi, 1.80, 0.30,
        f"L'utilisateur saisit\n{step}", fs=6.5)

y_last_a = 14.10 - (len(ajouter_steps) - 1) * 0.52

# valide
arr(ax, 10.5, y_last_a - 0.15, 10.5, y_last_a - 0.55, lw=0.8)
box(ax, 10.5, y_last_a - 0.70, 1.80, 0.28,
    "Sauvegarde dans\nnouveaux_matchs.csv", fs=6.5)
label_on_arrow(ax, 10.55, y_last_a - 0.33, "Saisie valide", fs=6.5)

# invalide
ax.annotate("", xy=(12.6, 14.10), xytext=(11.40, y_last_a + 0.15),
            arrowprops=dict(arrowstyle="-|>", color=ARR, lw=0.9,
                            connectionstyle="arc3,rad=-0.3"), zorder=2)
label_on_arrow(ax, 11.45, 13.65, "Saisie\ninvalide", fs=6.5)
box(ax, 12.85, 14.10, 1.5, 0.38,
    "Affichage\nmessage erreur\n→ annulation", fs=6.5)

# retour
ax.annotate("", xy=(7.0, 16.21), xytext=(10.5, y_last_a - 0.85),
            arrowprops=dict(arrowstyle="-|>", color=ARR, lw=0.9,
                            connectionstyle="arc3,rad=0.3"), zorder=2)
...
box(ax, 12.6, 13.65, 1.65, 0.46,
    "Lecture\nnouveaux_matchs.csv", fs=7.0)
arr(ax, 12.6, 13.42, 12.6, 12.96, lw=0.8)
box(ax, 12.6, 12.73, 1.65, 0.46,
    "Affichage liste\ntriée par date", fs=7.0)
ax.annotate("", xy=(7.0, 16.21), xytext=(12.6, 12.50),
            arrowprops=dict(arrowstyle="-|>", color=ARR, lw=0.9,
