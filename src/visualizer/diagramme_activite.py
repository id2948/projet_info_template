...
def decision(ax, cx, cy, size=0.38, texte="", fs=7.5):
    ...
    ax.add_patch(
        Polygon(pts, closed=True, facecolor=FC, edgecolor=EC, linewidth=LW, zorder=3)
    )
    if texte:
        ax.text(
            cx, cy, texte, ha="center", va="center", fontsize=fs, color=TXT, zorder=4
        )
...
def end_node(ax, cx, cy, r=0.18):
    ax.add_patch(plt.Circle((cx, cy), r, color=EC, zorder=5))
    ax.add_patch(
        plt.Circle((cx, cy), r + 0.10, color="none", ec=EC, lw=LW + 0.5, zorder=5)
    )
...
activity(ax, L_CX, Y3, AW, AH, "Saisit un sport\nou « q » pour quitter", fs=8.5)
...
label(ax, R_CX + 0.08, (Y_D2 - 0.38 + Y_D2 - 0.6) / 2, "Non", fs=7.8, color="#B71C1C")

Y9 = 3.30
arrow(ax, R_CX, Y_D2 - 0.38, R_CX, Y9 + AH / 2)
activity(ax, R_CX, Y9, AW, AH, "Afficher le message de fin\n« Au revoir »", fs=8.5)
