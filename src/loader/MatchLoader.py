        ...
        df = pd.DataFrame(
            {
                ...
                "equipe_1": df_games["team_id_home"]
                .astype(str)
                .map(teams)
                .fillna(df_games["team_id_home"].astype(str)),
                "equipe_2": df_games["team_id_away"]
                .astype(str)
                .map(teams)
                .fillna(df_games["team_id_away"].astype(str)),
        ...
        df = pd.DataFrame(
            {
                ...
                "equipe_1": df_matches["home_team_api_id"]
                .astype(str)
                .map(teams)
                .fillna(df_matches["home_team_api_id"].astype(str)),
                "equipe_2": df_matches["away_team_api_id"]
                .astype(str)
                .map(teams)
                .fillna(df_matches["away_team_api_id"].astype(str)),
        ...
        df_atp = pd.DataFrame(
            {
                ...
                "equipe_1": df_atp_m["winner_id"]
                .astype(str)
                .map(atp_players)
                .fillna(df_atp_m["winner_id"].astype(str)),
                "equipe_2": df_atp_m["loser_id"]
                .astype(str)
                .map(atp_players)
                .fillna(df_atp_m["loser_id"].astype(str)),
                "score_1": 1.0,
                "score_2": 0.0,
                "sport": "tennis",
        ...
        df_wta = pd.DataFrame(
            {
                ...
                "equipe_1": df_wta_m["winner_id"]
                .astype(str)
                .map(wta_players)
                .fillna(df_wta_m["winner_id"].astype(str)),
                "equipe_2": df_wta_m["loser_id"]
                .astype(str)
                .map(wta_players)
                .fillna(df_wta_m["loser_id"].astype(str)),
                "score_1": 1.0,
                "score_2": 0.0,
                "sport": "tennis",
...
class VolleyMatchLoader:
    ...
    def load_all_matches(self) -> list[Match]:
        ...
        df_men = df_men.rename(
            columns={"country_code_1": "code_1", "country_code_2": "code_2"}
        )
        df_women = df_women.rename(
            columns={"country_1": "code_1", "country_2": "code_2"}
        )
